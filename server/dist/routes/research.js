...
router.post('/submit', (req, res) => {
    const { employee_id, title, type, journal_name, impact_factor, authorship, date, submission_mode, attachment_path, issn_isbn, indexing, is_peer_reviewed, ugc_care_listed } = req.body;
...
router.get('/my/:employeeId', (req, res) => {
    ...
    db_1.default.all(`SELECT * FROM research_publications WHERE employee_id = ? ORDER BY date DESC`, [employeeId], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        res.json({ data: rows });
    });
...
router.get('/analytics', (req, res) => {
    ...
    const rankingSql = `
        ...
        LIMIT 10
    `;
    db_1.default.get(kpiSql, [], (err, kpis) => {
        if (err)
            return res.status(500).json({ error: err.message });
        db_1.default.all(rankingSql, [], (err, rankings) => {
            if (err)
                return res.status(500).json({ error: err.message });
            const analyticsData = kpis || { total: 0, approved: 0, totalCitations: 0, avgImpact: 0 };
            res.json({
                data: Object.assign(Object.assign({}, analyticsData), { rankings })
            });
        });
    });
});
...
router.patch('/review/:id', (req, res) => {
    ...
    db_1.default.run(sql, [status, reviewer_comments, approved_by, id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
