...
router.get('/config', (req, res) => {
    db_1.default.get("SELECT value FROM academic_config WHERE key = 'one_time_send'", (err, row) => {
        if (err)
...
router.post('/config', (req, res) => {
    ...
    db_1.default.run("INSERT OR REPLACE INTO academic_config (key, value, updated_at) VALUES ('one_time_send', ?, CURRENT_TIMESTAMP)", [JSON.stringify(config)], (err) => {
        if (err)
            return res.status(500).json({ error: err.message });
...
router.get('/groups', (req, res) => {
    db_1.default.all("SELECT * FROM academic_groups WHERE is_active = 1", [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        const data = rows.map(r => (Object.assign(Object.assign({}, r), { students: JSON.parse(r.students), teachers: JSON.parse(r.teachers) })));
        res.json({ data });
    });
});
...
router.post('/communications', (req, res) => {
    ...
    const uuid = `comm-${crypto_1.default.randomUUID()}`;
...
router.get('/communications', (req, res) => {
    ...
    db_1.default.all(sql, [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        let data = rows.map(r => (Object.assign(Object.assign({}, r), { recipient_groups: JSON.parse(r.recipient_groups), read_by: JSON.parse(r.read_by) })));
        ...
        if (role === 'STUDENT' && studentId) {
            ...
            db_1.default.all("SELECT uuid, students FROM academic_groups", [], (grpErr, groups) => {
                if (grpErr)
                ...
                data = data.filter(c => {
                    ...
                    const isForGroup = c.recipient_groups.some((rg) => studentGroups.includes(rg.groupId));
...
router.get('/communications/:uuid', (req, res) => {
    ...
    db_1.default.get("SELECT * FROM academic_communications WHERE uuid = ?", [uuid], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row)
            return res.status(404).json({ error: 'Communication not found' });
        res.json({
            data: Object.assign(Object.assign({}, row), { recipient_groups: JSON.parse(row.recipient_groups), read_by: JSON.parse(row.read_by) })
        });
    });
});
...
router.patch('/communications/:uuid', (req, res) => {
    ...
    db_1.default.get("SELECT deadline FROM academic_communications WHERE uuid = ?", [uuid], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row)
            return res.status(404).json({ error: 'Communication not found' });
        ...
        db_1.default.run(sql, [title, description, new Date(deadline).toISOString(), uuid], function (err) {
            if (err)
                return res.status(500).json({ error: err.message });
...
router.delete('/communications/:uuid', (req, res) => {
    const { uuid } = req.params;
    db_1.default.run("DELETE FROM academic_communications WHERE uuid = ?", [uuid], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
router.patch('/communications/:uuid/read', (req, res) => {
    ...
    db_1.default.get("SELECT read_by FROM academic_communications WHERE uuid = ?", [uuid], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row)
            return res.status(404).json({ error: 'Communication not found' });
        try {
            ...
            if (readBy.some((r) => r.studentId === studentId)) {
                return res.json({ message: 'Already marked as read' });
            }
            readBy.push({ studentId, readTime: new Date().toISOString() });
            db_1.default.run("UPDATE academic_communications SET read_by = ? WHERE uuid = ?", [JSON.stringify(readBy), uuid], (updateErr) => {
                if (updateErr)
...
router.get('/logs', (req, res) => {
    db_1.default.all("SELECT * FROM academic_communications ORDER BY sent_time DESC", [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
...
router.get('/reports', (req, res) => {
    db_1.default.all("SELECT * FROM academic_communications", [], (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        ...
        rows.forEach(r => {
            ...
            if (subject === "Mathematics")
                dept = "Mathematics";
            else if (subject === "English" || subject === "Hindi")
                dept = "Languages";
