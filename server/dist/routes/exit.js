    ...
    db_1.default.get(checkSql, [employee_id], (err, row) => {
        ...
        db_1.default.get(`SELECT value FROM configurations WHERE key = 'exit_config'`, (err, configRow) => {
            ...
            if (config.noticePeriod) {
                ...
                // Fallback defaults
                if (resignation_type === 'Retirement')
                    noticeDays = 90;
                else if (resignation_type === 'Contract End')
                    noticeDays = 30;
                else if (resignation_type === 'Termination')
                    noticeDays = 0;
            }
            ...
            db_1.default.get(`SELECT role FROM employees WHERE id = ?`, [employee_id], (err, emp) => {
                ...
                if (config.noticePeriod && emp) {
                    if (emp.role === 'Faculty')
                        noticeDays = config.noticePeriod.teachingDays;
                    else if (emp.role === 'Admin')
                        noticeDays = config.noticePeriod.nonTeachingDays; // Assuming Admin is non-teaching
                    else
                        noticeDays = config.noticePeriod.contractDays; // Fallback
                }
                if (resignation_type === 'Termination')
                    noticeDays = 0;
                ...
                db_1.default.run(sql, params, function (err) {
                    ...
                    // Audit Log
                    db_1.default.run(`INSERT INTO audit_logs (exit_id, action, performed_by, details) VALUES (?, ?, ?, ?)`, [exitId, 'Submitted', `Employee ${employee_id}`, `Resignation submitted. Type: ${resignation_type}, Proposed LWD: ${lwd_proposed}`]);
                    db_1.default.serialize(() => {
                        ...
                        db_1.default.all(`SELECT name, type FROM assets WHERE assigned_to = ?`, [employee_id], (err, assets) => {
                            if (err)
                                console.error("Error fetching assets for handover", err);
...
router.get('/config', (req, res) => {
    db_1.default.get(`SELECT value FROM configurations WHERE key = 'exit_config'`, (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row)
...
router.post('/config', (req, res) => {
    ...
    db_1.default.run(`INSERT OR REPLACE INTO configurations (key, value, updated_at) VALUES ('exit_config', ?, CURRENT_TIMESTAMP)`, [value], (err) => {
        if (err)
            return res.status(500).json({ error: err.message });
...
router.patch('/:id/status', (req, res) => {
    ...
    const { id } = req.params;
    db_1.default.get(`SELECT * FROM exits WHERE id = ?`, [id], (err, exit) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!exit)
        ...
        db_1.default.run(sql, params, function (err) {
            ...
            // Audit Log
            db_1.default.run(`INSERT INTO audit_logs (exit_id, action, performed_by, details) VALUES (?, ?, ?, ?)`, [id, status, approver_role || 'Admin', `Stage: ${exit.approval_stage} -> ${newStage}. Comments: ${comments}`]);
            ...
            if (finalLWD && exit.notice_period_end) {
                ...
                db_1.default.run(`UPDATE exits SET shortfall_days = ?, buyout_amount = ? WHERE id = ?`, [shortfall_days, buyout_amount, id]);
            }
...
router.patch('/:id/defer', (req, res) => {
    ...
    db_1.default.run(sql, [deferment_date, new Date().toISOString(), id], function (err) {
        if (err)
            return res.status(400).json({ error: err.message });
        // Audit Log
        db_1.default.run(`INSERT INTO audit_logs (exit_id, action, performed_by, details) VALUES (?, ?, ?, ?)`, [id, 'Deferred', 'Employee', `LWD extended to ${deferment_date}. Reason: ${reason}`]);
...
router.patch('/:id/modify', (req, res) => {
    ...
    const { id } = req.params;
    db_1.default.get(`SELECT status FROM exits WHERE id = ?`, [id], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row)
            return res.status(404).json({ error: "Exit not found" });
        if (row.status !== 'Pending')
            return res.status(400).json({ error: "Cannot modify processed resignation" });
        const sql = `UPDATE exits SET reason = ?, lwd_proposed = ? WHERE id = ?`;
        db_1.default.run(sql, [reason, lwd_proposed, id], function (err) {
            if (err)
                return res.status(400).json({ error: err.message });
            // Audit Log
            db_1.default.run(`INSERT INTO audit_logs (exit_id, action, performed_by, details) VALUES (?, ?, ?, ?)`, [id, 'Modified', 'Employee', `Resignation details updated. Reason: ${reason}, LWD: ${lwd_proposed}`]);
...
router.get('/:id', (req, res) => {
    const sql = `
        ...
        WHERE exits.id = ?
    `;
    db_1.default.get(sql, [req.params.id], (err, row) => {
        if (err) {
            return res.status(500).json({ error: err.message });
        }
        if (!row) {
        ...
        db_1.default.all(nocSql, [req.params.id], (nocErr, nocRows) => {
            if (nocErr) {
...
router.get('/:id/handover', (req, res) => {
    ...
    db_1.default.get(checkSql, [exitId], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (row.count === 0) {
            // No assets in handover yet, fetch from Assets table
            // Get employee_id from exit
            db_1.default.get(`SELECT employee_id FROM exits WHERE id = ?`, [exitId], (err, exitRow) => {
                if (err || !exitRow)
                    return res.status(500).json({ error: "Exit record not found" });
                const employeeId = exitRow.employee_id;
                const assetSql = `SELECT name, type FROM assets WHERE assigned_to = ?`;
                db_1.default.all(assetSql, [employeeId], (err, assets) => {
                    if (err)
                        return res.status(500).json({ error: err.message });
                    ...
                    const stmt = db_1.default.prepare(insertSql);
                    assets.forEach(asset => {
...
function fetchHandoverItems(res, exitId) {
    const sql = `SELECT * FROM handover_items WHERE exit_id = ?`;
    db_1.default.all(sql, [exitId], (err, rows) => {
        if (err) {
            return res.status(500).json({ error: err.message });
        }
        res.json({ data: rows });
...
router.patch('/handover/:id/verify', (req, res) => {
    ...
    db_1.default.run(`UPDATE handover_items SET status = 'Verified', last_updated = ? WHERE id = ?`, [new Date().toISOString(), req.params.id], function (err) {
        if (err)
            return res.status(400).json({ error: err.message });
...
router.patch('/:id/successor', (req, res) => {
    ...
    db_1.default.run(sql, [successor_id, id], function (err) {
        if (err)
            return res.status(400).json({ error: err.message });
        // Audit Log
        db_1.default.run(`INSERT INTO audit_logs (exit_id, action, performed_by, details) VALUES (?, ?, ?, ?)`, [id, 'Successor Assigned', 'Admin/Manager', `Successor ID ${successor_id} assigned.`]);
...
router.get('/:id/interview', (req, res) => {
    ...
    db_1.default.get(checkSql, [exitId], (err, row) => {
        ...
        if (row.count === 0) {
            ...
            const stmt = db_1.default.prepare(`INSERT INTO exit_interviews (exit_id, question, category, answer, rating) VALUES (?, ?, ?, '', 0)`);
            questions.forEach(item => stmt.run(exitId, item.q, item.cat));
            stmt.finalize();
        }
        db_1.default.all(`SELECT * FROM exit_interviews WHERE exit_id = ?`, [exitId], (err, rows) => {
            if (err)
                return res.status(500).json({ error: err.message });
            res.json({ data: rows });
        });
...
router.patch('/interview/:id', (req, res) => {
    ...
    db_1.default.run(`UPDATE exit_interviews SET answer = ?, rating = ? WHERE id = ?`, [answer, rating, req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
router.patch('/:id/interview/finalize', (req, res) => {
    ...
    db_1.default.run(sql, [risk_rating, hr_notes, interview_mode, req.params.id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
router.get('/analytics/dashboard', (req, res) => {
    ...
    db_1.default.all(`SELECT e.department, COUNT(*) as count FROM exits x JOIN employees e ON x.employee_id = e.id WHERE x.status = 'Approved' GROUP BY e.department`, (err, rows) => {
        if (err)
            return res.status(500).json({ error: err.message });
        analytics.attrition_by_dept = rows;
        // 2. Avg Ratings by Category
        db_1.default.all(`SELECT category, AVG(rating) as avg_rating FROM exit_interviews WHERE rating > 0 GROUP BY category`, (err, rows) => {
            if (err)
                return res.status(500).json({ error: err.message });
            analytics.sentiment_by_category = rows;
            // 3. Top Reasons (Word Cloudish)
            db_1.default.all(`SELECT reason, COUNT(*) as count FROM exits GROUP BY reason ORDER BY count DESC LIMIT 5`, (err, rows) => {
                if (err)
                    return res.status(500).json({ error: err.message });
                analytics.top_reasons = rows;
                // 4. Risk Profile
                db_1.default.all(`SELECT risk_rating, COUNT(*) as count FROM exits WHERE interview_status = 'Completed' GROUP BY risk_rating`, (err, rows) => {
                    if (err)
                        return res.status(500).json({ error: err.message });
                    ...
                    `, (err, row) => {
                        ...
                        db_1.default.all(`SELECT e.department, COUNT(*) as count 
                                 FROM exits x 
...
router.patch('/:id/calculate-shortfall', (req, res) => {
    const { id } = req.params;
    db_1.default.get(`SELECT lwd_approved, notice_period_end FROM exits WHERE id = ?`, [id], (err, exit) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!exit)
            return res.status(404).json({ error: "Exit not found" });
        ...
        db_1.default.run(sql, [shortfall_days, buyout_amount, id], function (err) {
            if (err)
                return res.status(400).json({ error: err.message });
...
router.patch('/:id/waiver', (req, res) => {
    ...
    db_1.default.run(sql, [reason, id], function (err) {
        if (err)
            return res.status(400).json({ error: err.message });
...
router.patch('/:id/waiver-approve', (req, res) => {
    ...
    db_1.default.serialize(() => {
        ...
        db_1.default.run(sql, [val, id], function (err) {
            if (err)
                return res.status(400).json({ error: err.message });
            if (approved) {
                db_1.default.run(`UPDATE exits SET buyout_amount = 0 WHERE id = ?`, [id]);
            }
...
router.post('/:id/settlement/calculate', (req, res) => {
    ...
    db_1.default.get(`SELECT exits.*, employees.* 
            FROM exits 
            LEFT JOIN employees ON exits.employee_id = employees.id 
            WHERE exits.id = ?`, [id], (err, record) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!record)
            return res.status(404).json({ error: 'Exit not found' });
        ...
        db_1.default.get(`SELECT id FROM final_settlements WHERE exit_id = ?`, [id], (err, existing) => {
            ...
            if (existing) {
                ...
                db_1.default.run(insertSql, params, function (err) {
                    ...
                    db_1.default.run(`UPDATE exits SET settlement_status = 'In Progress' WHERE id = ?`, [id]);
...
router.post('/:id/settlement/approve', (req, res) => {
    ...
    const sql = `UPDATE final_settlements SET 
        ...
        updated_at = ?
        WHERE exit_id = ?`;
    db_1.default.run(sql, [approved_by || 1, currentDate, currentDate, id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
        // Update exits table
        db_1.default.run(`UPDATE exits SET settlement_status = 'Completed'  WHERE id = ?`, [id]);
...
router.post('/:id/settlement/process', (req, res) => {
    ...
    const sql = `UPDATE final_settlements SET 
        ...
        updated_at = ?
        WHERE exit_id = ?`;
    db_1.default.run(sql, [currentDate, payment_reference, new Date().toISOString(), id], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
router.post('/:id/settlement/notify', (req, res) => {
    ...
    const sql = `SELECT fs.*, employees.name, employees.email, exits.lwd_approved
                 ...
                 LEFT JOIN employees ON exits.employee_id = employees.id
                 WHERE fs.exit_id = ?`;
    db_1.default.get(sql, [id], (err, settlement) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!settlement)
