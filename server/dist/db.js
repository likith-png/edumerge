...
const db = new sqlite3_1.default.Database('./hrms.db', (err) => {
    if (err) {
...
function initTables() {
    db.serialize(() => {
      ...
      updated_at TEXT DEFAULT CURRENT_TIMESTAMP
        ...
        db.get(`SELECT count(*) as count FROM configurations WHERE key = "onboarding_stages"`, (err, row) => {
            if (!row || row.count === 0) {
                const defaultStages = [
        ...
        db.get(`SELECT count(*) as count FROM configurations WHERE key = 'exit_config'`, (err, row) => {
            if (!row || row.count === 0) {
                const defaultConfig = {
                    general: {
      ...
      FOREIGN KEY (vehicle_id) REFERENCES vehicles(id)
        ...
        db.get("SELECT count(*) as count FROM vehicles", (err, row) => {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
                ...
                const fut60 = new Date(today);
                fut60.setDate(today.getDate() + 60);
                const fut20 = new Date(today);
                fut20.setDate(today.getDate() + 20);
                const fut5 = new Date(today);
                fut5.setDate(today.getDate() + 5);
                const past = new Date(today);
                past.setDate(today.getDate() - 10);
                const fut365 = new Date(today);
                fut365.setFullYear(today.getFullYear() + 1);
                const dFmt = (d) => d.toISOString().split('T')[0];
        ...
        db.get("SELECT count(*) as count FROM research_publications", (err, row) => {
      ...
      updated_at TEXT DEFAULT CURRENT_TIMESTAMP
        ...
        // Seed groups
        db.get("SELECT count(*) as count FROM academic_groups", (err, row) => {
            if (!err && row.count === 0) {
        ...
        // Seed config
        db.get("SELECT count(*) as count FROM academic_config WHERE key = 'one_time_send'", (err, row) => {
        ...
        // Seed communications
        db.get("SELECT count(*) as count FROM academic_communications", (err, row) => {
            if (!err && row.count === 0) {
                ...
                stmt.run("comm-1", "homework", "Solve exercises 5.1 to 5.10", "Solve all exercises from textbook chapter 5. Submit by tomorrow at 5 PM. Show all steps.", "Mathematics", "emp-001", "John Faculty", JSON.stringify([
                    { groupId: "group-10a", groupName: "Class 10-A", studentCount: 45 },
                    { groupId: "group-10b", groupName: "Class 10-B", studentCount: 42 }
                ]), "2026-06-10T14:30:00.000Z", "2026-06-11T17:00:00.000Z", JSON.stringify(readBy1));
                stmt.run("comm-2", "classwork", "Complete assignment on fractions", "Complete worksheet 3 on fractions. Turn it in before leaving class.", "Mathematics", "emp-001", "John Faculty", JSON.stringify([
                    { groupId: "group-10a", groupName: "Class 10-A", studentCount: 45 }
                ]), "2026-06-09T10:15:00.000Z", "2026-06-10T15:00:00.000Z", JSON.stringify(readBy2));
                stmt.run("comm-3", "homework", "Physics Lab Report - Optics", "Draw ray diagrams for concave and convex mirrors. Record focal lengths and write conclusions.", "Science", "hod-001", "Dr. Rajesh (HOD)", JSON.stringify([
                    { groupId: "group-10b", groupName: "Class 10-B", studentCount: 42 },
                    { groupId: "group-10c", groupName: "Class 10-C", studentCount: 38 }
                ]), "2026-06-08T16:45:00.000Z", "2026-06-12T17:00:00.000Z", JSON.stringify([]));
                stmt.finalize();
            }
        });
