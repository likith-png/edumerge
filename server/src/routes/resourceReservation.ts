...
function formatDisplayDate(dateMs: number): string {
...
function expandReservation(resv: { start_ts: string; end_ts: string; recurrence: string }) {
...
// Checks a candidate booking against a list of existing active bookings and/or other candidate bookings
interface ConflictQueryParam {
  resource_id: number;
...
async function checkConflicts(
  candidates: ConflictQueryParam[],
  ignoreIds: number[] = []
): Promise<{ conflict: boolean; clashDetails: { date: string; message: string; resourceName: string }[] }> {
  return new Promise((resolve, reject) => {
    // 1. Fetch all resources details to get buffer_minutes and name
    db.all("SELECT id, name, is_splittable, buffer_minutes FROM resources", [], (err, resources: any[]) => {
      ...
      db.all(query, [], (err, dbReservations: any[]) => {
        if (err) return reject(err);
        ...
        const clashDetails: { date: string; message: string; resourceName: string }[] = [];
        ...
        candidates.forEach((cand, candIdx) => {
          ...
          candidates.forEach((otherCand, otherIdx) => {
            if (candIdx >= otherIdx) return; // avoid double checks and self-check
...
router.get('/resources', (req, res) => {
  ...
  db.all(resourceQuery, [], (err, resources: any[]) => {
    if (err) return res.status(500).json({ error: err.message });

    db.all(sectionQuery, [], (err, sections: any[]) => {
      if (err) return res.status(500).json({ error: err.message });
      ...
      const result = resources.map(r => {
        ...
        return {
          ...r,
          is_splittable: r.is_splittable === 1,
          internal_only: r.internal_only === 1,
          approval_flow: parsedFlow,
          rate_card: r.hourly_rate !== null ? {
            hourly_rate: r.hourly_rate,
            daily_rate: r.daily_rate,
            security_deposit: r.security_deposit
          } : null,
          sections: sections.filter(s => s.resource_id === r.id)
        };
      });
  ...
  db.run(sql, [
    ...
    if (!internal_only) {
      ...
      `, [
        ...
        if (err) console.error("Error creating rate card: ", err);
      });
    }
...
router.get('/reservations', (req, res) => {
  ...
  db.all(resvQuery, [], (err, reservations: any[]) => {
    if (err) return res.status(500).json({ error: err.message });

    db.all("SELECT * FROM approval_steps ORDER BY level ASC", [], (err, steps: any[]) => {
      if (err) return res.status(500).json({ error: err.message });

      db.all("SELECT * FROM collection_demands", [], (err, demands: any[]) => {
        if (err) return res.status(500).json({ error: err.message });

        db.all("SELECT * FROM override_logs", [], (err, logs: any[]) => {
          if (err) return res.status(500).json({ error: err.message });

          db.all("SELECT * FROM event_refs", [], (err, events: any[]) => {
            if (err) return res.status(500).json({ error: err.message });

            const result = reservations.map(r => {
              return {
                ...r,
                approval_steps: steps.filter(s => s.reservation_id === r.id),
                collection_demand: demands.find(d => d.reservation_id === r.id) || null,
                override_log: logs.find(l => l.reservation_id === r.id) || null,
                event_ref: events.find(e => e.reservation_id === r.id) || null
              };
            });
...
router.post('/check-conflicts', async (req, res) => {
  ...
  try {
    const result = await checkConflicts(bookings, ignoreIds);
    res.json(result);
  } catch (err: any) {
...
router.post('/book', async (req, res) => {
  ...
  try {
    ...
    const check = await checkConflicts(bookings);
    if (check.conflict) {
    ...
    db.serialize(() => {
      ...
      db.all("SELECT * FROM rate_cards", [], (err, rateCards: any[]) => {
        if (err || insertErrors) {
          db.run("ROLLBACK");
        ...
        db.all("SELECT id, approval_flow FROM resources", [], (err, resourcesList: any[]) => {
          if (err || insertErrors) {
            db.run("ROLLBACK");
...
router.post('/approve', (req, res) => {
  ...
  db.run(updateStepSql, [status, comments || null, reservation_id, level], function (this: any, err: any) {
    if (err) return res.status(500).json({ error: err.message });

    // Handle rejection
    if (status === 'Rejected') {
      db.run("UPDATE reservations SET status = 'Rejected' WHERE id = ?", [reservation_id], (err) => {
        if (err) return res.status(500).json({ error: err.message });
    ...
    db.all(checkStepsSql, [reservation_id, level], (err, pendingSteps: any[]) => {
      if (err) return res.status(500).json({ error: err.message });
      ...
      if (pendingSteps && pendingSteps.length > 0) {
        ...
        db.run("UPDATE reservations SET status = 'Approved' WHERE id = ?", [reservation_id], (err) => {
          if (err) return res.status(500).json({ error: err.message });
...
router.post('/override-cancel', (req, res) => {
  ...
  db.get(getDetailsSql, [reservation_id], (err, resv: any) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!resv) return res.status(404).json({ error: "Reservation not found" });

    // 2. Update reservation status to Cancelled
    db.run("UPDATE reservations SET status = 'Cancelled' WHERE id = ?", [reservation_id], (err) => {
      if (err) return res.status(500).json({ error: err.message });
      ...
      `, [reservation_id, reason_code, reason_text, cancelled_by], (err) => {
        ...
        db.get("SELECT * FROM collection_demands WHERE reservation_id = ?", [reservation_id], (err, demand: any) => {
          if (err) return res.status(500).json({ error: err.message });

          // Cancel the demand if present
          if (demand) {
            db.run("UPDATE collection_demands SET status = 'Cancelled' WHERE id = ?", [demand.id]);
          }

          // 5. Fetch linked EventRef
          db.get("SELECT * FROM event_refs WHERE reservation_id = ?", [reservation_id], (err, event: any) => {
            if (err) return res.status(500).json({ error: err.message });
...
router.get('/utilization', (req, res) => {
  ...
  db.all(query, [], (err, resources: any[]) => {
    if (err) return res.status(500).json({ error: err.message });

    db.all("SELECT * FROM reservations WHERE status = 'Approved'", [], (err, approvals: any[]) => {
      if (err) return res.status(500).json({ error: err.message });

      db.all("SELECT * FROM collection_demands WHERE status != 'Cancelled'", [], (err, demands: any[]) => {
        if (err) return res.status(500).json({ error: err.message });
